/* features/settingsLink.js — Clones the sidebar "Profile" link and
   re-labels the copy as "Settings" (pointing at /settings). Keeps the
   nav styling consistent with Inleo's own look, and re-runs each
   watchdog tick so the link survives SPA re-renders. */
(function () {
    const NS = (window.InleoSkins = window.InleoSkins || {});
    const utils = NS.utils;

    const LINK_ID = 'inleo-skin-settings-link';
    ...
    function inject() {
        ...
        /* Find the actual Profile link by reading visible text, not class names. */
        ...
        /* Clear any active-route styling inherited from the Profile node. */
